exports.run = {
   ...
   async: async (m, {
      ...
      if (command == 'putus') {
         ...
         if (!text && !m.quoted) return client.reply(m.chat, Func2.texted('bold', `🚩 Mention or Reply chat target.`), m)
         if (isNaN(number)) return client.reply(m.chat, Func2.texted('bold', `🚩 Invalid number.`), m)
         if (number.length > 15) return client.reply(m.chat, Func2.texted('bold', `🚩 Invalid format.`), m)
         try {
            if (text) {
            ...
            if (user == client.user.id.split(':')[0] + 's.whatsapp.net') return client.reply(m.chat, Func2.texted('bold', `🚩 The fuck?`), m)
            if (typeof target == 'undefined') return client.reply(m.chat, Func2.texted('bold', `🚩 Can't find user data.`), m)
            if (user == m.sender) return client.reply(m.chat, Func2.texted('bold', `🚩 Stress ??`), m)
            if (command == 'tembak') {
