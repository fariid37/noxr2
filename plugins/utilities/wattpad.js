   ...
   async: async (m, {
      ...
      try {
         if (command == 'wattpad') {
            ...
            client.sendListv2(m.chat, '', `Menampilkan hasil dari pencarian untuk : “${text}“, silahkan pilih judul di bawah ini untuk menampilkan informasi, dan daftar part cerita. 🍟`, '', 'Tap!', rows, m)
         } else if (command == 'wattpadget') {
            if (!args || !args[0]) return
            let json = await WP.detail(args[0])
            if (!json.status) return client.reply(m.chat, global.status.fail, m)
            let caption = `乂 *W A T T P A D*\n\n`
