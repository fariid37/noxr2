      ...
      try {
         ...
         if (room) {
            ...
            if (m.sender == room.p2 && /(acc|reject)/.test(command) && m.isGroup && room.status == 'wait') {
               ...
               let infos = `Apabila menang akan mendapat *+${Func.formatNumber(room.poin)} Point* dan apabila kalah Point mu dikurangi sebesar *-${Func2.formatNumber(room.poin_lose)}*`
