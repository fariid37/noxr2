   ...
   try {
      ...
      if ((body && body.length != 1 && (isPrefix = (myPrefix || '')[0])) || body && commands.includes((body.split` ` [0]).toLowerCase())) {
         ...
         for (let name in is_commands) {
            ...
            if (!m.isGroup && !['owner', 'confess', 'create_bot'].includes(name) && chats && !isPrem && !users.banned && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 Untuk bisa menggunakan bot di pribadi chat kamu harus upgrade ke *premium* atau gabung grup dibawah ini jika kalian ingin gratis, tetapi kamu hanya bisa menggunakan bot ini di grup saja. \n\njika kamu ingin upgrade ke premium, ketik *${prefixes[0]}owner*`, m, {
               largeThumb: true,
         ...
         for (let name in is_events) {
            ...
            if (!m.isGroup && chats && !isPrem && !users.banned && !['chatAI'].includes(name) && setting.groupmode) return client.sendMessageModify(m.chat, `🚩 Untuk bisa menggunakan bot ini di pribadi chat kamu harus upgrade ke *premium* atau gabung grup dibawah ini jika ingin gratis, tetapi kamu hanya bisa menggunakan bot di grup saja.\n\njika kamu ingin upgrade ke premium, ketik *${prefixes[0]}owner*`, m, {
               largeThumb: true,
               thumbnail: await Func.fetchBuffer('https://telegra.ph/file/0b32e0a0bb3b81fef9838.jpg'),
               url: setting.link
            }).then(() => chats.lastchat = new Date() * 1)
